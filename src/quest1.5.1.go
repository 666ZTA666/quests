...
func main() {
	...
	var n = Timeout1()
	//создаем контекст с таймаутом. В предыдущем файле контекст был с таймаутом
	ctx, cancel := context.WithDeadline(context.TODO(), time.Now().Add(time.Duration(n)*time.Second))
	defer cancel()
	fmt.Println("запускаем секундомер")
	start := time.Now() // записываем текущее время в переменную старт
	myChan := make(chan int)
	go myFunc1(ctx, myChan) // запускаем функцию с контекстом, которая проверяет ошибку из контекста,
...
func myFunc1(ctx context.Context, myChan chan int) {
	var j int
	for j < math.MaxInt {
