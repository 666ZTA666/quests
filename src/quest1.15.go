...
// взятие не 100 символов, а 100 байт, что в рунах можжет быть куда меньше 100 символов
...
//Либо, если нам нужны 100 первых байт из строки, то сохранять их лучше в срез байт.
// И изменяя v(allStrings) с которой сделан срез, мы будем менять justString.
...
func someFunc() (justString string) {
	...
	allString := createHugeString(1 << 10)
	justString1 := allString[:100] //Разберем вариант, где нам нужно первых 100 сиволов-рун
	justString = string(append([]rune{}, []rune(allString)[:100]...))
	// tmp := make([]rune, 100)
